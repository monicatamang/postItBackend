...
def update_comment():
    ...
    # Update the user's comment with the given the login token, comment id and content
    ...
    # If the user's content is updated, get the updated comment from the database
    if(row_count == 1):
        ...
        # If the updated comment is not retrieved from the database, send a server error response
        else:
            return Response("Database Error. Please refresh the page.", mimetype="text/plain", status=500)
    # If the user's comment is not updated, send a server error response
    else:
